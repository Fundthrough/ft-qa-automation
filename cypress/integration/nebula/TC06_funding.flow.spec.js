...
describe('Invoice funding flow for new sign up', () => {
    ...
    it('Validate the funding flow for new sign up', () => {
        ...
        function getPaid() {
            cy.get('.eligible_invoice').each($el => {
                $el.get('.invoice__price').text().then()
                cy.wrap($el).signinElements.customerPage.getenteredinvoice().invoke('text').then($invoice_amount => {
                    cy.wrap($el).signinElements.customerPage.getinvoicecustomer().invoke('text').then($customer_name => {
                        cy.wrap($el).signinElements.customerPage.getinvoicename().invoke('text').then($invoice_name => {
                            cy.wrap($el).signinElements.customerPage.getduedate().invoke('text').then($invoice_due_date => {
                                cy.wrap($el).signinElements.customerPage.getpaidbutton().then($ele => {
                                    cy.wrap($ele).click()
                                    ...
                                    signinElements.customerPage.getforwardbutton().click()
                                    cy.url().should('include', '/invoices')
                                    signinElements.customerPage.getreviewingstatus().invoke('text').then($text => {

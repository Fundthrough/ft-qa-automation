...
export class InvoiceDetails {
    verifyInvoiceDetails() {
        ...
        cy.wait(2000)
        cy.get('[name="all"]').should('be.visible').click()

        cy.get(invoicePanelSelectors.invoiceTable).then((body) => {
    ...
    invoiceDetails() {
        cy.get(invoicePanelSelectors.customerName)
            .invoke('text')
            .then((customerName) => {
                cy.get(invoicePanelSelectors.invoiceNumber)
                    .invoke('text')
                    .then((invoiceNumber) => {
                        const [invoice, number] = invoiceNumber.split('#')
                        const invoicenumber = invoice + '#' + ' ' + number
                        cy.get(invoicePanelSelectors.invoicePrice)
                            .invoke('text')
                            .then((invoicePrice) => {
                                cy.get(invoicePanelSelectors.dueDate)
                                    .invoke('text')
                                    .then((dueDate) => {
                                        ...
                                        const duedate =
                                            month + ' ' + date + ' ' + year
                                        cy.get(
                                            invoicePanelSelectors.invoiceState
                                        )
                                            .invoke('text')
                                            .then((state) => {
                                                cy.get(
                                                    invoicePanelSelectors.customerName
                                                ).click()
                                                cy.get(
                                                    invoicePanelSelectors.panel
                                                ).should('be.visible')
                                                cy.get(
                                                    invoicePanelSelectors.panelHeader
                                                ).should(
                                                    'contain.text',
                                                    customerName
                                                )
                                                cy.get(
                                                    invoicePanelSelectors.panelNumber
                                                ).should(
                                                    'contain.text',
                                                    invoicenumber
                                                )
                                                cy.get(
                                                    invoicePanelSelectors.panelDetails
                                                ).should(
                                                    'contain.text',
                                                    invoicePrice
                                                )
                                                ...
                                                cy.get(
                                                    invoicePanelSelectors.panelTitles
                                                )
                                                    .eq(0)
                                                    .should(
                                                        'contain.text',
                                                        invoicePanelTexts.value
                                                    )
                                                cy.get(
                                                    invoicePanelSelectors.panelTitles
                                                )
                                                    ...
                                                    .should(
                                                        'contain.text',
                                                        invoicePanelTexts.due
                                                    )

                                                if (state == 'Get Paid') {
                                                    cy.get(
                                                        invoicePanelSelectors.panelState
                                                    )
                                                        .should(
                                                            'contain.text',
                                                            invoicePanelTexts.eligible
                                                        )
                                                        .and(
                                                            'have.css',
                                                            'text-transform',
